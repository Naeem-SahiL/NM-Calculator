...
//    console.log(Y)
   ...
   Y.map((val, i)=>{
    ...
    for(var j = 0; j < no - i; j++)
        delta[j][i] = math.round((delta[j + 1][i - 1] - delta[j][i - 1]),4);
    ...
    var sum = delta[floor(no/2)][0];
    var p = (value - X[floor(no/2)]) / (X[1] - X[0])
    
    for(i = 1; i < no; i++)
    {
        sum = sum + (p_cal(p, i) * delta[floor((no-i)/2)][i]) / fact(i)
    }
    Setans(sum.toFixed(4))
    SetShow(true);
  };
