...
//    console.log(Y)
   ...
   for ( i = 1; i < no; i++) {
    for (var j = no - 1; j >= i; j--)
        delta[j][i] = delta[j][i - 1] - delta[j - 1][i - 1];
}
    ...
    var sum = delta[no - 1][0];
    var u = (value - X[no - 1]) / (X[1] - X[0]);
    for ( i = 1; i < no; i++) {
        sum = sum + (ucal(u, i) * delta[no - 1][i]) /
                                     fact(i);
    }
